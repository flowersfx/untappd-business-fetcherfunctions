...
namespace FlowersFX
{
    public static class GetSpiritMenu
    {
        static IConfigurationRoot config;
        static Utilities utilities;

        [FunctionName("GetSpiritMenu")]
            ...
            [TimerTrigger("0 45 * * * *", RunOnStartup = true)]TimerInfo myTimer, ILogger log, ExecutionContext context)
        {
            ...
            if (config["UNTAPPD_SPIRIT_MENU_ID"] == null)
            {
                return;
            }
            ...
            var filename = config["BLOB_STORAGE_FILE_NAME_PREFIX"] + "-spirit-menu.json";
        ...
        public static async Task<MenuRoot> GetUntappdCustomMenu(IConfigurationRoot config)
        {
            ...
            var url = $"https://business.untappd.com/api/v1/menus/{untappedSpiritMenuId}?full=true&include_legacy=true";
