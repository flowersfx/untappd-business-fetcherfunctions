...
namespace FlowersFX
{
    public static class GetFoodMenuSwedish
    {
        static IConfigurationRoot config;
        static Utilities utilities;

        [FunctionName("GetFoodMenuSwedish")]
        public static async Task Run(
            [TimerTrigger("0 46 * * * *", RunOnStartup = true)]TimerInfo myTimer, ILogger log, ExecutionContext context)
        {
            ...
            var filename = config["BLOB_STORAGE_FILE_NAME_PREFIX"] + "-food-menu-swedish.json";
        ...
        {
            var untappdFoodMenuId = config["UNTAPPD_FOOD_MENU_SWEDISH_ID"];
            var url = $"https://business.untappd.com/api/v1/menus/{untappdFoodMenuId}?full=true&include_legacy=true";
